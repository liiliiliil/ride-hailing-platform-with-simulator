...
save_file_name = '20161102_experiment_dist_150'
save_data = True
n_time_units = 144
max_n_drivers = 150
max_dispatch_dist = 6
...
order_dispatch = OrderDispatch(value_map, grids_dist_table, extra_time_for_pickup, max_dispatch_dist)
...
if len(orders) > 0:
    match_order_to_grid, weight = order_dispatch.dispatch_with_only_distance(state[0].reshape(-1), orders, moment)
...
if save_data:
    ...
    with open(os.path.join(save_path, save_file_name+'_result.pkl'), 'wb') as f:
        pickle.dump({'response_rate': order_response_rates, 'profit': total_profit}, f)
