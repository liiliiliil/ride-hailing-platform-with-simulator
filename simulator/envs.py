...
class CityReal:
    ...
    def __init__(self, mapped_matrix_int, order_num_dist, idle_driver_dist_time, idle_driver_location_mat, order_time_dist, order_price_dist,
                 ...
                 global_flag="global", time_interval=10, extra_time_for_pickup=None):
        """
        :param mapped_matrix_int: 2D matrix: each position is either -100 or grid id from order in real data.
        :param order_num_dist: len is 144, like [{node_id1: [mu, std], node_id2: [mu, std]}, {node_id1: [mu, std], node_id2: [mu, std]}, ...]
        ...
        :param idle_driver_location_mat: 144 x num_valid_grids matrix. # each element is mean of idle drivers at particular grid
        ...
        :param n_side:
        :param time_interval:
        :param l_max: The max-duration of an order, the duration means the number of time slots required for order completion
        :return:
        """
        ...
        self.city_time = 0  # keeps increasing while the local variable 'moment' will be city_time % n_intervals
        ...
        self.target_grids = []  # list, each element is the index for getting node, and the index is got from matrix like [[0, 1, 2, ...], [...], ...]
        self.n_valid_grids = 0  # num of valid grid
        self.nodes = [None for _ in range(self.M * self.N)]  # list, each element is node object, and node._index is corresponding to the map or matrix like [[0, 1, 2, ...], [...], ...] (not mapped_matrix_int)
        ...
        self.idle_driver_dist_time = idle_driver_dist_time  # list, len is 144, each element is [mean, std] of idle drivers at particular time
        self.idle_driver_location_mat = idle_driver_location_mat  # matrix, 144 rows, num_valid_grid cols, each element is mean of idle drivers at particular grid

        self.order_time_dist = order_time_dist[:l_max]/np.sum(order_time_dist[:l_max])  # list, each element is the prop that order's duration is (ind+1). Since the max duration is 9, len of this list is 9.
        self.order_price_dist = order_price_dist  # list, similar to order_time_dist but each element is [mean, std]
        ...
        for item in target_grids_sorted:
            ...
            target_node_ids.append(ids_2dto1d(x, y, M, N)[0])  # x, y is np.narray, so the return of ids_2dto1d function is also np.narray with only one element and the index is necessary then.
        self.target_node_ids = target_node_ids
        # store valid note id. Sort by number of orders emerged. descending.

        self.node_mapping = {}  # key is index from mapped_matrix_int and value is index from matrix like [[0, 1, 2, ...], [...], ...] (same shape with mapped_matrix_int)
        ...
        self.out_grid_in_orders = np.zeros((self.n_intervals, len(self.target_grids)))  # consider the orders that starts at grids out of map but ends at grids in the map.
        self.global_flag = global_flag
        self.weights_layers_neighbors = [1.0, np.exp(-1), np.exp(-2)]

        if extra_time_for_pickup is None:
            # distance of [0,1,2] with 0 extra time for pickup, [3,4,5] with 1, [6,7,8] with 2 and so on
            self.extra_time_for_pickup = np.floor(np.arange(self.mapped_matrix_int.shape[0]+self.mapped_matrix_int.shape[0])/3).astype(int)
        else:
            self.extra_time_for_pickup = extra_time_for_pickup
        
        ### for order dispatch ###
        self.order_index_to_node = None
        self.order_index_to_index_in_node = None
    ...
    def construct_node_real(self, mapped_matrix_int):
        ...
        """todo: I may optimize codes here (2 loops may not necessary)"""
        for x, y in zip(row_inds, col_inds):
            node_id = ids_2dto1d(x, y, self.M, self.N)
    ...
    def get_observation_with_extra_order_info(self):
        next_state = np.zeros((2, self.M, self.N))
        # node_index_to_orders = [None] * self.n_nodes
        orders = []
        self.order_index_to_node = []
        self.order_index_to_index_in_node = []
        count = 0
        for _node in self.nodes:
            if _node is not None:
                row_id, column_id = ids_1dto2d(_node.get_node_index(), self.M, self.N)
                ...
                # node_index_to_orders[_node.get_node_index()] = _node.orders

                # order is [start_grid_id, end_grid_id, start_time_unit, duration_time_unit, price]
                orders.extend([order.get_info_list() for order in _node.orders])
                self.order_index_to_node.extend([_node]*_node.order_num)
                self.order_index_to_index_in_node.extend(list(range(_node.order_num)))
                count += _node.order_num

        assert count == np.sum(next_state[1])

        return next_state, orders
    ...
    def reset_clean(self, generate_order=1, ratio=1, city_time=""):
        ...
        print('len(self.day_orders) is', len(self.day_orders))
        ...
        print('num_idle_driver is', num_idle_driver)
        self.step_driver_online_offline_control_new(num_idle_driver)
        return self.get_observation()
    
    def reset_clean_with_extra_order_info(self, generate_order=1, ratio=1, city_time=""):
        """ 1. bootstrap oneday's order data.
            2. clean current drivers and orders, regenerate new orders and drivers.
            can reset anytime
        :return:
        ...
        print('num_idle_driver is', num_idle_driver)
        self.step_driver_online_offline_control_new(num_idle_driver)
        return self.get_observation_with_extra_order_info()
    ...
    def utility_set_drivers_offline_real_new(self, n_drivers_to_off):
        ...
        n_drivers_can_be_off = int(np.sum(curr_idle_driver_distribution_resort[np.where(idle_diff >= 0)]))  # idle_diff > 0 or the np.where is useless here
    ...
    def utility_bootstrap_oneday_order(self):
        ...
        print('self.n_intervals is', self.n_intervals)
        for iorder in one_day_orders:
            ...
            if iorder[0] not in self.node_mapping.keys() and iorder[1] not in self.node_mapping.keys():
                continue
            start_node = self.node_mapping.get(iorder[0], -100)  # -100 is default
            end_node = self.node_mapping.get(iorder[1], -100)
            duration = int(iorder[3])
            price = iorder[4]
            waiting_time = iorder[5]

            # if order does not start in the map, then at the time the order is comp
            if start_node == -100:
                column_index = self.target_grids.index(end_node)
                self.out_grid_in_orders[(start_time + duration) % self.n_intervals, column_index] += 1
                continue

            if end_node not in self.target_grids:
                # print('In utility_bootstrap_oneday_order: end_node will be None. iorder[1] is', iorder[1])
                end_node = None

            day_orders[start_time].append([start_node, end_node, start_time, duration, price, waiting_time])
    ...
    def step_driver_status_control(self):
        ...
        moment = self.city_time % self.n_intervals
        orders_to_on_drivers = self.out_grid_in_orders[moment, :]  # get a list here, each element is number of drivers arrive at particular grid
    ...
    def step_driver_online_offline_nodewise(self):
        ...
        for idx, target_node_id in enumerate(self.target_node_ids):
            ...
            # print('on_off_number is', on_off_number)
            if on_off_number > 0:
    ...
    def step_driver_online_offline_control_new(self, n_idle_drivers):
        ...
        # print('n_idle_drivers is', n_idle_drivers)
        # print('self.n_offline_drivers is', self.n_offline_drivers)
    ...
    def utility_get_n_idle_drivers_real(self):
        ...
        """
        time = self.city_time % self.n_intervals
        # print('self.idle_driver_location_mat[time] is', self.idle_driver_dist_time[time])
    ...
    def step_bootstrap_order_real_with_pickup_time_and_waiting_time(self, day_orders_t):
        for iorder in day_orders_t:
            # print('len(iorder) is', len(iorder))
            start_node_id = iorder[0]
            end_node_id = iorder[1]
            start_node = self.nodes[start_node_id]
            ...
            if end_node_id in self.target_grids:
                ...
                # print('end_node will be None. This end_node_id is', end_node_id)
                end_node = None
            start_node.add_order_real_with_pickup_time_and_waiting_time(self.city_time, end_node, iorder[3], iorder[4], iorder[5])
    ...
    def step_assign_order_broadcast_neighbor_reward_update(self):
        ...
        if all_order_num != 0:
            # print('finished_order_num is', finished_order_num, 'and all_order_num is', all_order_num)
            self.order_response_rate = finished_order_num/float(all_order_num)
        else:
            self.order_response_rate = -1
    ...
    def step_assign_order_with_pickup_time_and_waiting_time(self, match_order_to_grid):
        """ Consider the orders whose destination or origin is not in the target region
        :param num_layers:
        :param weights_layers_neighbors: [1, 0.5, 0.25, 0.125]
        :return:
        ...
        served_orders_indexes = [[] for _ in range(self.n_nodes)]
        served_orders_number = [0] * self.n_nodes

        for i in range(len(match_order_to_grid)):
            node_ind = self.order_index_to_node[i].get_node_index()
            flag, reward = self.order_index_to_node[i].assigin_order_to_grid(self.order_index_to_index_in_node[i], match_order_to_grid[i], self.city_time, self)
            if flag:
                # print('order %d in grid %d has been assigined' % (self.order_index_to_index_in_node[i], node_ind))
                served_orders_indexes[node_ind].append(self.order_index_to_index_in_node[i])
                served_orders_number[node_ind] += 1

                node_reward[node_ind] += reward

        for node in self.nodes:
            node_ind = node.get_node_index()
            # print('clean orders in grid %d' % node_ind)
            node.clean_orders(served_orders_indexes[node_ind])
        
        
        if len(match_order_to_grid) != 0:
            # print('finished_order_num is', finished_order_num, 'and all_order_num is', all_order_num)
            self.order_response_rate = np.sum(served_orders_number)/len(match_order_to_grid)
        else:
            self.order_response_rate = -1

        return np.sum(node_reward), node_reward
    ...
    def step_dispatch_invalid_more_neighbors(self, dispatch_actions):
        """ If a
        :param dispatch_actions:
        :return:
        """
    ...
    def step(self, dispatch_actions, generate_order=1):
        ...
        save_remove_id = self.step_dispatch_invalid(dispatch_actions)  # move drivers (fleet management), if the action is valid, corresponding drivers will be set offline temporarily
        # When the drivers go to invalid grid, set them offline.

        reward, reward_node = self.step_assign_order_broadcast_neighbor_reward_update() # order dispatching

        ############################ observation ############################
        now_state = self.get_observation()
        print("City time {}: Order response rate: {}".format(self.city_time, self.order_response_rate))
        # order_response_rates.append(env.order_response_rate)

        print("idle driver: {} == {} total num of drivers: {}".format(np.sum(now_state[0]),
                                                                        np.sum(self.get_observation_driver_state()),
                                                                        len(self.drivers.keys())))
        
        print("number of unassigned orders: {} total number of orders: {}".format(np.sum(now_state[1]),  # it is the state of next time unit, so the orders have not been dispatched
                                                                        len(self.day_orders[self.city_time % self.n_intervals])))
        ...
        # generate order at t + 1
        if generate_order == 1:
            self.step_generate_order_real()  # randomly generate order
        else:
            moment = self.city_time % self.n_intervals
            self.step_bootstrap_order_real(self.day_orders[moment])  # add orders at this moment using orders sampled from real data at the beginning
        
        # offline online control;
        self.step_driver_online_offline_nodewise()  # randomly change the on-off state of some drivers

        # # control the number distribution of drivers, added by myself
        # # Init current driver distribution
        # if self.global_flag == "global":
        #     num_idle_driver = self.utility_get_n_idle_drivers_real()
        #     num_idle_driver = int(num_idle_driver)
        # else:
        #     num_idle_driver = self.utility_get_n_idle_drivers_nodewise()
        # self.step_driver_online_offline_control_new(num_idle_driver)

        self.step_remove_unfinished_orders()  # cancel orders that cannot wait anymore
        # get states S_{t+1}  [driver_dist, order_dist]
        next_state = self.get_observation()  # in this time, i.e. first half time of next time unit, orders have not been dispatched, so the order number is number of not unfinished or undispatched orders but the total number

        context = self.step_pre_order_assigin(next_state)  # np.narray, shape is [2, M, N], each element is the remaining drivers/orders in the map after considering the neighbors that can be assigned with orders
        info = [reward_node, context]
        return next_state, reward, info


    def step_with_pickup_time_and_waiting_time(self, dispatch_actions, match_order_to_grid):
        info = []
        '''**************************** T = 1 ****************************'''
        
        reward, reward_node = self.step_assign_order_with_pickup_time_and_waiting_time(match_order_to_grid) # order dispatching

        # Loop over all dispatch action, change the driver distribution
        save_remove_id = self.step_dispatch_invalid_more_neighbors(dispatch_actions)  # move drivers (fleet management), if the action is valid, corresponding drivers will be set offline temporarily
        # When the drivers go to invalid grid, set them offline.


        ############################ observation ############################
        now_state = self.get_observation()
        print("City time {}: Order response rate: {} Profit: {}".format(self.city_time, self.order_response_rate, reward))
        # order_response_rates.append(env.order_response_rate)

        # print("idle driver: {} == {} total num of drivers: {} num of offline drivers {}".format(np.sum(now_state[0]),
        #                                                                 np.sum(self.get_observation_driver_state()),
        #                                                                 len(self.drivers.keys()),
        #                                                                 len(self.utility_collect_offline_drivers_id())))
        ...
        # generate order at t + 1
        moment = self.city_time % self.n_intervals
        self.step_bootstrap_order_real_with_pickup_time_and_waiting_time(self.day_orders[moment])  # add orders at this moment using orders sampled from real data at the beginning
        ...
        self.step_remove_unfinished_orders()  # cancel orders that cannot wait anymore
        # get states S_{t+1}  [driver_dist, order_dist]
        next_state, orders = self.get_observation_with_extra_order_info()  # in this time, i.e. first half time of next time unit, orders have not been dispatched, so the order number is number of not unfinished or undispatched orders but the total number

        # context = self.step_pre_order_assigin(next_state)  # np.narray, shape is [2, M, N], each element is the remaining drivers/orders in the map after considering the neighbors that can be assigned with orders
        # info = [reward_node, context]
        return next_state, orders, reward

# def step_control_idle_drivers_number(self, dispatch_actions, generate_order=1):
#         info = []
#         '''**************************** T = 1 ****************************'''
#         # Loop over all dispatch action, change the driver distribution
#         save_remove_id = self.step_dispatch_invalid(dispatch_actions)  # move drivers (fleet management), if the action is valid, corresponding drivers will be set offline temporarily
