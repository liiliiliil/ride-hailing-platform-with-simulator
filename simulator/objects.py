...
class Node(object):
    ...
    def add_order_real_with_pickup_time_and_waiting_time(self, city_time, destination_node, duration, price, waiting_time):
        current_node_id = self.get_node_index()
        self.orders.append(Order(self,
                                 destination_node,
                                 city_time,
    ...
    def remove_unfinished_order(self, city_time):
        ...
        for idx, o in enumerate(self.orders):
            ...
            # order completed  (this part seems unnecessary)
    ...
    def assigin_order_to_grid(self, order_index, grid_index, city_time, city):

        reward = 0
        
        order_to_serve = self.orders[order_index]
        dest_node = city.nodes[grid_index]
        
        
        
        for key, assigned_driver in dest_node.drivers.items():
            if assigned_driver.onservice is False and assigned_driver.online is True:
                if order_to_serve.get_end_position() is not None:
                    assigned_driver.take_order(order_to_serve)
                    removed_driver = dest_node.drivers.pop(assigned_driver.get_driver_id(), None)
                    assert removed_driver is not None
                else:
                    assigned_driver.set_offline()  # order destination is not in target region
                city.n_drivers -= 1
    ...
    def clean_orders(self, served_order_index):

        # remove served orders
        self.orders = [i for j, i in enumerate(self.orders) if j not in served_order_index]
        # if self.order_num != len(self.orders):
        #     print(self.order_num, len(self.orders))
        #     print(served_order_index)
        assert self.order_num == len(self.orders)
    ...
    def simple_order_assign_broadcast_update(self, city, neighbor_node_reward):
        ...
        for neighbor_node in self.neighbors:  # #1, or index 0, neighbor always picks first
    ...
    def status_control_eachtime(self, city):
        ...
        if self.onservice is True:
            ...
            if self.city_time == order_end_time:
                ...
                print('city_time is', self.city_time, 'and order_end_time is', order_end_time)
...
class Order(object):
    ...
    def __init__(self, begin_position, end_position, begin_time, duration, price, wait_time):
        ...
        self._pickup_time = -1  # added by liiliiliil
    ...
    def get_info_list(self):
        ...
        # if self._begin_p is None:
        #     print('start node is None:', self._begin_t, self._t, self._p, self._waiting_time)
        # if self._end_p is None:
        #     print('destination node is None:', self._begin_t, self._t, self._p, self._waiting_time)
        # if self._begin_t is 144:
        #     print('begin time is 144:', self._begin_t, self._t, self._p, self._waiting_time)
        return [self._begin_p.get_node_index(), self._end_p.get_node_index(), 
                self._begin_t, self._t, self._p, self._waiting_time]
